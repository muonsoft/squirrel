...
Captured before fork implementation changes. These files preserve the
...
| `muonsoft-fork-api.txt` | muonsoft/squirrel | post-cleanup fork | regenerated after public API normalization |
...
- `root-modules-before-cleanup.txt` — fork root graph before test dependency cleanup.
- `root-modules-after-cleanup.txt` — post-cleanup graph, containing only the fork,
