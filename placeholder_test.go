...
func TestAtp(t *testing.T) {
	...
	assertEqual(t, "x = @p1 AND y = @p2", s)
}

func TestPlaceholders(t *testing.T) {
	t.Parallel()
	assertEqual(t, "?,?", Placeholders(2))
}
...
func TestEscapeDollar(t *testing.T) {
	...
	s, _ := Dollar.ReplacePlaceholders(sql)
	assertEqual(t, "SELECT uuid, \"data\" #> '{tags}' AS tags FROM nodes WHERE  \"data\" -> 'tags' ?| array['$1'] AND enabled = $2", s)
}
...
func TestEscapeAtp(t *testing.T) {
	...
	sql := "SELECT uuid, \"data\" #> '{tags}' AS tags FROM nodes WHERE  \"data\" -> 'tags' ??| array['?'] AND enabled = ?"
	s, _ := AtP.ReplacePlaceholders(sql)
	assertEqual(t, "SELECT uuid, \"data\" #> '{tags}' AS tags FROM nodes WHERE  \"data\" -> 'tags' ?| array['@p1'] AND enabled = @p2", s)
}
